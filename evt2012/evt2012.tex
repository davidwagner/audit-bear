...
not aim to detect or prevent these; in this work we assume that DRE audit logs
are complete, accurate, trustworthy, and free of accidental or malicious
tampering. Detecting and preventing audit log tampering is outside the scope of
...
\item{Personalized Electronic Ballot (PEB).} The PEB is a proprietary cartridge
  ...
  delivered to the precinct with no ballot style information which is later supplied 
  by the PEB. The PEBs are programmed at election central with the ballot data to be 
  used at the predetermined voting locations. The precinct’s ballot definition is 
  stored in the PEB’s internal flash memory. At the opening of the polls, pollworkers 
  download ballot style information from the PEB to the terminal.  When the PEB is 
  placed in the 
...
\item{Activator PEBs.} Activator PEBs are used by poll workers to activate ballots
  ...
  each precinct with multiple activator PEBs. Internally, all PEBs at the precinct 
  are identical. The only difference between them and the Master PEB  is the color 
  of the rubber band on their exterior. Thus, a Master PEB can be used to activate 
  a voter's ballot and an Activator PEB can be used to open and close terminals. Though 
  as a matter of procedure and training, they should not be used this way. If an Activator 
  PEB is used to close terminals, the precinct vote totals may be partially uploaded to the 
  aggregated totals on election night. Poll workers are trained to put the Master PEB, CF 
  ...
  vote data not added to the certified count.
\end{itemize}
...
amenable to this type of analysis is discussed in Section~\ref{sec:suggestions}.
...
what time the totals report was printed, this may not be indicative of the time that 
...
refer to these tapes to identify polling locations open late, then they would 
experience many false positives. Additionally, this allows a more convenient way for 
election officials to find out which precincts were open after 7 P.M. In order to 
